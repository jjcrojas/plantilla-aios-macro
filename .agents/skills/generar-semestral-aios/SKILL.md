...
description: Prepara la Plantilla AIOS-probable.xlsm para un corte semestral ejecutando su rutina no interactiva de actualización y después ejecuta la aplicación Spring Boot plantilla-aios-macro para generar y verificar semestral.xlsx. Usar cuando el usuario solicite generar, regenerar o procesar un informe semestral con corte al 30 de junio o 31 de diciembre.
---
...
4. Abrir una instancia aislada y oculta de Excel, escribir la fecha en `CARATULA!B2` y ejecutar la macro no interactiva `ActualizarSeriesSinPortapapeles`.
5. Esperar el recálculo, validar las celdas de `cuentas` que consume el semestral y guardar la plantilla solo si la preparación termina correctamente.
...
Para validar dependencias y datos de la plantilla sin guardar cambios ni generar el archivo:
...
Entregar la ruta absoluta de `semestral.xlsx`, el período validado, el tamaño y la ruta del respaldo. Por defecto se escribe en `target/aios-output/semestral-AAAA-MM/semestral.xlsx`.

Si falla la macro, falta el período en las hojas base, Excel abre la plantilla como solo lectura, la aplicación no inicia o el libro generado no supera las validaciones, informar el error y las rutas de los logs. No afirmar que el semestral fue generado.
