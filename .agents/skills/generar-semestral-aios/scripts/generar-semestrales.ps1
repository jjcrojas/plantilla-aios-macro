...
if ($fechasCorte.Count -eq 0) {
    throw "El rango $Desde a $Hasta no contiene cortes semestrales."
}
...
$singleScript = Join-Path $PSScriptRoot 'generar-semestral.ps1'
if (-not (Test-Path -LiteralPath $singleScript -PathType Leaf)) {
    throw "No se encontró el generador semestral individual: $singleScript"
}
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $OutputDir = Join-Path $projectPath "target\aios-output\semestrales-$Desde-a-$Hasta"
}
...
if ($SoloValidar) {
    Write-Output "VALIDACION_SEMESTRALES_OK desde=$Desde hasta=$Hasta periodos=$($fechasCorte.Count)"
    return
}
...
function Test-SemestralesWorkbook {
    ...
    try {
        ...
        if (@($workbookXml.workbook.sheets.sheet).Count -lt 1) {
            throw 'El archivo semestral consolidado no contiene hojas.'
        }
        ...
        $labels = foreach ($cutoff in $Cutoffs) {
            $month = if ($cutoff.Month -eq 6) { 'junio' } else { 'diciembre' }
            "$month-$($cutoff.ToString('yyyy'))"
        }
        foreach ($cutoff in $Cutoffs) {
            $month = if ($cutoff.Month -eq 6) { 'junio' } else { 'diciembre' }
            if ($content.IndexOf($month, [StringComparison]::OrdinalIgnoreCase) -lt 0 -or
                    $content.IndexOf($cutoff.ToString('yyyy'), [StringComparison]::OrdinalIgnoreCase) -lt 0) {
                throw "No se encontró el período $month $($cutoff.Year) en el semestral consolidado."
            }
        }
        foreach ($formulaError in @('#REF!', '#DIV/0!', '#VALUE!', '#NAME?', '#N/A')) {
            if ($content.IndexOf($formulaError, [StringComparison]::OrdinalIgnoreCase) -ge 0) {
                throw "Se encontró el error de fórmula $formulaError en el semestral consolidado."
            }
        }
        if ([regex]::Matches($content, '<v>-?\d+(?:\.\d+)?</v>').Count -lt 10) {
            throw 'El archivo semestral consolidado no contiene suficientes valores numéricos.'
        }
        return ($labels -join ',')
...
try {
    ...
    $uri = "$baseUrl/aios/generar-rango?desde=$desdeCorte&hasta=$hastaCorte&modo=SEMESTRAL"
    Invoke-WebRequest -Uri $uri -Method Post -OutFile $destination -TimeoutSec 3600 -UseBasicParsing
    $file = Get-Item -LiteralPath $destination
    if ($file.Length -eq 0) { throw "La generación semestral de $Desde a $Hasta produjo un archivo vacío." }
    $periodLabels = Test-SemestralesWorkbook -Path $file.FullName -Cutoffs $fechasCorte
    Write-Output "SEMESTRALES_GENERADOS_OK desde=$Desde hasta=$Hasta periodos=$periodLabels bytes=$($file.Length) ruta=$($file.FullName)"
    $file.FullName
} finally {
